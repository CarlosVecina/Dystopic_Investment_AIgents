...
from dystopic_investment_aigents.data_ingestion.download_yh_ticker_data import (
...
def load_data(*args, **kwargs):
    ...
    response_dict = download_ticker_data(
        tickers=symbol_list,
        start_date=kwargs["execution_date"] - datetime.timedelta(days=4),
        end_date=kwargs["execution_date"] - datetime.timedelta(days=1),
    )

    df = (
        pd.concat(response_dict, axis=0)
        .reset_index(level=0)
        .rename({"level_0": "ticker"}, axis=1)
        .sort_index(ascending=False)
    )

    return df.head(df["ticker"].nunique()).reset_index()
